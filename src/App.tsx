...
import { Bed, Patient, Stats } from './types';
...
const BedManagement = () => {
  ...
  const [newBed, setNewBed] = useState({ id: '', type: 'Normal' });
...
const Admission = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setMessage(`Success! Patient assigned to Bed ${data.bedId}`);
...
const PatientList = () => {
  ...
  const handleDischargeClick = (patient: Patient) => {
    ...
    setAmountPaid(patient.amount_paid || 0);
  };
  ...
  const confirmDischarge = async () => {
    ...
    const amountDue = totalBill - amountPaid;
    ...
    if (res.ok) {
      ...
      if (data.status === 'Payment Pending') {
        alert('Patient moved to Pending Payments list.');
      } else {
            ...
            >
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="flex justify-between items-center p-4 bg-slate-50 rounded-xl">
                  ...
                  <span className={cn(
                    "text-xl font-bold",
                    calculateTotalBill(billingPatient) - amountPaid > 0 ? "text-rose-600" : "text-emerald-600"
                  )}>
                    ₹{Math.max(0, calculateTotalBill(billingPatient) - amountPaid).toLocaleString()}
                  </span>
                </div>
  ...
  const confirmPayment = async () => {
    ...
    const res = await fetch('/api/discharge', {
      ...
      body: JSON.stringify({ 
        ...
        status: isFullPayment ? 'Discharged' : 'Payment Pending'
      }),
    });
    
    // For this demo, we'll just update local state directly
    setBillingPatient(null);
    if (isFullPayment) {
      setPatients(patients.filter(p => p.id !== billingPatient.id));
      alert('Payment complete. Patient discharged successfully.');
    } else {
      setPatients(patients.map(p => p.id === billingPatient.id ? { ...p, amount_paid: (p.amount_paid || 0) + amountPaid, amount_due: remainingDue } : p));
